...
def get_place_details(query: str, language_code: str = "ja"):
    ...
    """
    # ① Place ID を取得
    search_result = gmaps.find_place(input=query, input_type="textquery", fields=["place_id", "name"], language=language_code)
    
    if search_result.get("status") == "OK" and search_result.get("candidates"):
        place_id = search_result["candidates"][0]["place_id"]
        print(f"[Place ID 取得成功] {search_result['candidates'][0]['name']} - {place_id}")
    else:
